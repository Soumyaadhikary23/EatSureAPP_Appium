...
public class D2C_VR_ESAPP_EndtoEnd_SANITY extends BaseTest {
	SoftAssert as;
	public static final Logger log = LogManager.getLogger(D2C_VR_ESAPP_EndtoEnd_SANITY.class.getName());
...
//		locality.scrollToText(prop.getProperty("location"));
	...
	public void verifyProductDetailsPage() throws Throwable {
		...
		restPage.clickonFirstProductToNavigatetoPDPPage();
		Thread.sleep(2000);
...
//		log.info("Total product in the cart " + cart.productCount());
	...
	@Test(enabled = false, priority = 14, description = "Verify the address")
	...
	@Test(enabled = true, priority = 15, description = "Verify the coupon")
	public void coupon() throws Throwable {
		CartPage cart = new CartPage(driver);
		LocalityPage locality = new LocalityPage(driver);
		cart.scrollToText("View all");
...
//		as.assertAll();
	...
	@Test(enabled = true, priority = 16, description = "Verify free dish ")
